...
class TutorialPipeline(object):
    ...
    def process_item(self, item, spider):
        # sql操作
        insert_sql = 'insert into news_news (title,article_content,created_time,last_modified_time,article_image,category,status,views) values ("{}","{}","{}","{}","{}","{}","{}","{}")'.format(item['article_title'], item['article_content'], datetime.datetime.now(), datetime.datetime.now(), item['article_image'], item['article_type'], 'd', 0)
